import { Controller, Get, Query, Res, Logger, Post, Body, Delete, Param } from '@nestjs/common';
...
export class SocialController {
	...
	async callbackFacebook(@Query('code') code: string, @Res() res) {
		const frontend = this.config.get<string>('FRONTEND_URL') || 'http://localhost:9000';
	...
	async callbackTwitter(@Query('code') code: string, @Query('state') state: string, @Res() res) {
		const frontend = this.config.get<string>('FRONTEND_URL') || 'http://localhost:9000';
		...
		try {
			...
			const saved = await this.social.persistTwitterAccount(result.user, result.token?.access_token);
	...
	async uploadImage(@Body() body: UploadImageDto, @Res() res) {
		try {
			...
			const publicBase = this.config.get<string>('BACKEND_PUBLIC_URL') || this.config.get<string>('BACKEND_URL') || 'http://localhost:9001';
